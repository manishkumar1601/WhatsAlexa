...
let msg = '*This Plugin is Officially Approved!* ✅'
let unmsg = '*This Plugin isn\'t Officially Approved!* ❌'
...
WhatsAlexa.addCommand({pattern: 'insert ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        if (find.includes(plugin_name)) {
            ...
            if (!match[1].includes('TOXIC-DEVIL') && DEG.level > 99) {
                await message.client.sendMessage(message.jid,LANG.limit + DEG.level + '_', MessageType.text)
                fs.unlinkSync('/root/WhatsAlexa/plugins/' + plugin_name + '.js')
            }
            else if (!match[1].includes('TOXIC-DEVIL') && DEG.level < 100) {
